...
const VideoCallProvider = lazy(() =>
    import('./features/chat/context/VideoCallContext').then((module) => ({ default: module.VideoCallProvider }))
)
...
const FOOTER_PATHS = ["/", "/login", "/verify-otp", "/forgot-password", "/reset-password"];
...
const Body = () => {
    ...
    const shouldSkipProfileQuery = AUTH_PUBLIC_PATHS.includes(location.pathname);

    const { data: profile, error } = useGetProfileQuery(undefined, { skip: shouldSkipProfileQuery });
